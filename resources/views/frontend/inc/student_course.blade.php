<div class="contentCard contentCard--course h-100 justify-content-between">
    ...
    <div class="contentCard-bottom">
        ...
        <div class="contentCard-info d-flex align-items-center justify-content-between">
            <a href="@auth @if ($course->instructor_id == Auth::user()->id)
                {{route('profile.myprofile', Auth::user()->id)}}
            @else
                {{route('profile.instructor', $course->instructor_id)}}
            @endif
            @endauth
            @guest
            {{route('profile.instructor', $course->instructor_id)}}
            @endguest" class="contentCard-user d-flex align-items-center">
                @if ($course->instructor_photo == null)
                <img src="{{asset('images/avatar.svg')}}" alt="Instructor">
                @else
                <img src="{{asset($course->instructor_photo)}}" alt="client-image" class="rounded-circle" style="width: 23%;">
                @endif
